...
const liveMemoryService = require("./liveMemory.service");
...
exports.runAgentQuery = async ({ userId, query, topK = 5, memoryTypes = [], saveReply = false }) => {
  ...
  const response = await groq.chat.completions.create({
    ...
    // Optionally tune max tokens etc in config
  });
  ...
  if (saveReply && reply) {
    try {
      savedMemory = await memoryService.saveMemory({
        ...
        metadata: { userId, sourceQuery: query, usedMemoryIds: usedMemories.map(m => m.id) }
      });
    } catch (err) {
